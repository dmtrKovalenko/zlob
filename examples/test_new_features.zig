...
pub fn main() !void {
    ...
    var result = glob_libc.glob("{src,examples}/*.zig", glob_libc.GLOB_BRACE, null, &pglob);
    if (result == 0) {
    ...
    result = glob_libc.glob("src/*.{zig,c}", glob_libc.GLOB_BRACE, null, &pglob);
    if (result == 0) {
        std.debug.print("  SUCCESS: Matches: {d}\n", .{pglob.gl_pathc});
        var i: usize = 0;
        while (i < pglob.gl_pathc and i < 10) : (i += 1) {
    ...
    std.debug.print("\nTest 3: Recursive glob **/*.zig\n", .{});
    result = glob_libc.glob("**/*.zig", 0, null, &pglob);
    if (result == 0) {
        std.debug.print("  SUCCESS: Matches: {d}\n", .{pglob.gl_pathc});
        var i: usize = 0;
    ...
    result = glob_libc.glob("src/**/*.zig", 0, null, &pglob);
    if (result == 0) {
        std.debug.print("  SUCCESS: Matches: {d}\n", .{pglob.gl_pathc});
        var i: usize = 0;
        while (i < pglob.gl_pathc and i < 10) : (i += 1) {
    ...
    result = glob_libc.glob("{src,examples}/**/*.zig", glob_libc.GLOB_BRACE, null, &pglob);
    if (result == 0) {
        std.debug.print("  SUCCESS: Matches: {d}\n", .{pglob.gl_pathc});
        var i: usize = 0;
        while (i < pglob.gl_pathc and i < 15) : (i += 1) {
